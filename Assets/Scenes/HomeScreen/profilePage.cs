...
public class profilePageButton : MonoBehaviour
{
    ...
    private void Start()
    {
        // on start, we access the current profile chosen, if not found we set a default picture
        string currentProfile = PlayerPrefs.GetString("profilePic", "picture01");
        // find the image from the array of profilePictures
        int index = Array.IndexOf(profiles, currentProfile);
        // we enable to profile selected
